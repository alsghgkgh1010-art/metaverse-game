...
for(const 게 of 게임){
  ...
  if(게.지도검사){
    ...
    확인(문제은행.length >= 20, `문항 ${문제은행.length}개 (족자보다 많아야 매번 다른 문제가 나온다)`);
    ...
    지도.forEach((rows, i) => {
      ...
      const 필수수 = 문제은행.filter(x => x.마당 === i+1 && x.필수).length;
      if(문항수 < 세기('Q'))  bad(`${이름} 족자 ${세기('Q')}개 > 문항 ${문항수}개 — 채울 문제가 모자란다`);
      if(필수수 > 세기('Q')) bad(`${이름} 반드시 내야 할 문항 ${필수수}개 > 족자 ${세기('Q')}개`);
      총족자 += 세기('Q');
